...
/** 주행 Mapillary 거리뷰 패널 `top` — 좌상단 menu·search(2.4rem) 하단 + 6px, 세로 중첩 방지 */
const MAPILLARY_RIDE_STREET_PANEL_TOP = `calc(${SAFE_TOP_1REM} + 2.4rem + 6px)`;
...
const App: React.FC = () => {
  ...
  }, [mapillaryBasicCoverageVisible, isMapReady, mapType, flushMapillaryCoverageToMap]);
  ...
  useEffect(() => {
    ...
    const pedalRpm = useSensorCadence
      ? Math.min(CRANK_RPM_MAX, Math.max(CRANK_RPM_MIN, emaRpm))
      : estimateCrankRpmFromSpeedKmh(speedNow);
    const pedalLoopSec = Math.max(0.22, Math.min(5.5, 60 / pedalRpm));
    const pedalingRunning = simulation.isActive && speedNow > 0.35;
    ...
    if (!simulationMarker.current && map && mbGl) {
      ...
      if (flip) {
        ...
        if (!stack) {
          ...
          stack = buildCyclingSimMarkerStack(pedalLoopSec, pedalingRunning);
    ...
    setMapillaryBasicCoverageVisible((prev) => {
      ...
      if (next && !MAPILLARY_CLIENT_TOKEN) {
        setMapBootstrapError(
          'Mapillary 촬영 노선을 지도에 표시하려면 VITE_MAPILLARY_CLIENT_TOKEN이 필요합니다. 프로젝트 루트 .env.local 에 토큰을 넣고 개발 서버(npm run dev)를 다시 실행하세요.'
        );
        return false;
      }
  ...
  return (
    <div className="fixed inset-0 bg-slate-900 overflow-hidden font-sans">
      ...
      {rideMapillaryStreet && route?.path?.length ? (
        <div
          className="fixed z-[1005] pointer-events-auto flex min-h-0 flex-col overflow-hidden rounded-xl border border-slate-600/80 bg-black/90 shadow-2xl max-h-[min(50dvh,300px)] landscape:max-h-[min(82dvh,440px)]"
          style={{
            left: SAFE_LEFT_1REM,
            top: MAPILLARY_RIDE_STREET_PANEL_TOP,
        ...
        >
          ...
          <div className="relative min-h-[100px] w-full flex-1 overflow-hidden bg-black">
            <MapillaryRideViewer
