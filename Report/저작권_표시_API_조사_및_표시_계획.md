...
**최종 갱신**: 자문위원 의견 반영 (저작권 표시에 대한 자문 청취 후 계획 정리)
...
## 4. 저작권 표시 계획 (자문위원 의견 반영)

### 4.1 표시 원칙

- **Google**: API 기본 저작권 영역을 가리지 않고, 필요 시 앱 내 “지도/거리뷰 출처” 문구 보조 표기. *(자문: 기본 UI 가림 금지 유지)*  
- **OpenStreetMap / Nominatim / OSRM**: ODbL 요구에 따라 **반드시** “OpenStreetMap” (및 가능하면 © contributors, 링크) 표기. *(자문: 법적 필수 표기로 우선 적용)*  
- **Open-Elevation**: 정책이 명확하지 않으나 **Credits** 에 서비스명·URL 표기 권장. *(자문: 선의 표기, 정보 화면에 두어 UI 단순화)*  

### 4.2 표시 위치·방식 제안 (자문: 표시 단순화·가독성)
...
| 3 | **앱 푸터 또는 About** | PWA/웹 앱이면 푸터/About에 “Map data © OpenStreetMap, © Google” 등 요약 *(선택)* | 전체 요약 |

- **자문 반영**: 맵 하단은 **한 줄·간결**하게, 상세 출처는 **정보(Credits) 화면 한 곳**에 두어 사용자가 찾기 쉽고 화면이 지나치게 복잡해지지 않도록 함.
...
- **OpenStreetMap**  
  ...
  - 맵과 같은 화면에서 **읽기 쉽게** 유지(작은 글씨라도 대비·크기 확보). *(자문: 가독성 확보)*  
...
저작권 표시에 대해 자문위원 의견을 청취하였고, 아래 요지를 반영하여 본 계획을 정리·업데이트하였다.

| 구분 | 자문 의견 요지 | 계획 반영 내용 |
|------|----------------|----------------|
| **필수 vs 권장** | 법적 필수(OSM)와 권장(Google 보조, Open-Elevation) 구분을 명확히 할 것 | 4.1·6.요약에서 OSM 필수, Google 기본 UI 유지, Open-Elevation 선의 표기로 구분 명시 |
| **표시 단순화** | 맵 위 표기는 한 줄로 간결하게, 상세는 한 곳(정보 화면)으로 모을 것 | 4.2에서 맵 하단 한 줄 + 정보/Credits 화면 상세로 정리, 4.2 하단 자문 반영 문구 추가 |
| **가독성** | 저작권 문구가 사용자에게 읽히도록 크기·대비 확보 | 4.4 OSM 항목에 “읽기 쉽게 유지” 명시 및 자문 반영 표기 |
| **Google 기본 UI** | API가 제공하는 저작권·로고 영역을 가리거나 제거하지 말 것 | 2.1·4.1·4.4에 유지, 4.5 체크리스트에 화면별 점검 포함 |
| **적용 순서** | 필수(OSM 맵 인근 표기) 먼저, 이어서 정보 화면·선택 항목 적용 | 4.2 우선순위표 및 4.5 체크리스트 순서로 반영 |

이상 자문 의견을 반영한 저작권 표시 계획이다. 구현 시 4.5 체크리스트 순으로 진행하면 된다.
...
저작권 표시 계획의 **핵심**은 (1) **OpenStreetMap** 에 대한 표기를 **맵/경로를 보는 화면 인근**에 반드시 두는 것, (2) **Google** 기본 표기를 가리지 않는 것, (3) **상세 출처**는 정보/Credits 화면에 두는 것이다. *(자문위원 의견 반영 완료)*
