...
- **Google**: 현재와 동일하게 **API 기본 저작권 영역을 그대로 유지**한다. 수정·제거·가림 없음.
- **그 외(OpenStreetMap / Nominatim / OSRM / Open-Elevation)**: 기본맵에는 표시 공간이 부족하므로, **초기 로딩 화면**과 **About** 으로 분리해 표기한다.

### 4.2 표시 위치·방식

- **Google**: API 기본 저작권 영역을 가리지 않고, 필요 시 앱 내 “지도/거리뷰 출처” 문구 보조 표기.- **OpenStreetMap / Nominatim / OSRM**: ODbL 요구에 따라 **반드시** “OpenStreetMap” (및 가능하면 © contributors, 링크) 표기.- **Open-Elevation**: 정책이 명확하지 않으나 **Credits** 에 서비스명·URL 표기 권장.
...
|-----|----------------|----------------|
| Google Maps & Street View | 예 (기본 UI 유지) | 맵 기본 UI 그대로, 변경 없음 |
| Nominatim / OSRM (OSM 데이터) | **예 (필수)** | 초기 로딩 화면 하단 요약 + About 상세 |
| Open-Elevation | 권장(선의) | 초기 로딩 화면 하단 요약 + About 상세 |

**방향 요약**: Google 저작권은 **현행 유지**. 그 외 저작권은 기본맵에 공간을 두지 않고, **초기 로딩 화면 하단**에 요약 표기, **About** 링크는 맵 하단 Google 표기 **왼쪽**에 두어 상세 출처를 제공한다.